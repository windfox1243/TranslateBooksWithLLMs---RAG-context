...
NOVEL_CONTEXT_PROMPT_MAX_TOKENS=3000
...
**Using auto novel context?** The full context file remains editable and durable, but prompts receive only scene-relevant accepted context so the system prompt stays cacheable. Selection is always active; `NOVEL_CONTEXT_PROMPT_MAX_TOKENS` caps the selected bundle, which is what makes ranking and the dynamic-state budget reserve bite - the default 3000 leaves room for a crowded scene, and `0` turns the cap off entirely. Keep `NOVEL_CONTEXT_UPDATE_INTERVAL=1` for maximum consistency, or raise it to reduce extra context-analysis calls. `NOVEL_CONTEXT_SOURCE_MEMORY_CHARS` gives the analyzer a bounded tail of previous source chunks so facts split across chunk boundaries can still be resolved.
