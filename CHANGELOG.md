...
- The Senior Editor is told again what counts as a defect. `editor-issue-v7-focused` tightened the contract by deleting its enumeration of defect classes — omission, mistranslation, glossary error, pronoun bleed, gender mismatch, register, consistency, fluency, style, placeholder and format — and replaced it with the circular instruction to report issues when issues exist. Three one-way valves landed in the same change (`at most 12 material issues`, `omit equally valid alternatives`, `confidence below 0.80 is review_only`), and between them the cheapest answer that still satisfied the contract became `no_issues`. A small editor model took it: in a 33-chunk book it returned a byte-identical 27-token empty verdict for 31 of them, and every defect repaired in that book had been found by the deterministic checks, not by the editor. The checklist is back as an explicit audit the model walks before answering, the strict JSON schema is unchanged, and `category` must now name the checklist item the issue came from. The contract version is `editor-issue-v8-enumerated`, stamped on every `editor_runs` row, so a job's database says which contract judged it.

### Added

- `EDITOR_LOG_FULL_RESPONSE=true` prints the editor's raw answer verbatim instead of the three-bullet TL;DR. The summary condenses an empty verdict and a thorough audit to the same nothing, which is why the contract regression above ran for four weeks unseen; the logs could not distinguish an editor that found nothing from an editor that looked for nothing. Off by default — a full critique is thousands of characters per chunk.

### Changed

- The Senior Editor's automatic thinking level is now `low` rather than `minimal`, and never zero. The editor is the only pass that reads the source beside the draft, so its entire value is the comparison, and `minimal` bought it no room to make one. This is not what silenced the editor — the contract was, see above — but a model floored at `minimal` spent zero thinking tokens across two whole books, and a comparison it is not given time to make is not one it can report. Models that offer no `low` rung take the lowest they do offer, so the floor cannot resolve to a level the provider rejects. Gemini 2.5 editors, which were getting thinking switched off outright, now get the smallest budget instead. An explicitly chosen editor thinking level is untouched; only `auto` moves. Expect the editor to cost more per chunk: thinking tokens are now actually spent, and the automatic editor output budget doubles with the higher level.
