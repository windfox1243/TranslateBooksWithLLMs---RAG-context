...
- The Senior Editor's automatic thinking level is now `low` rather than `minimal`, and never zero. The editor is the only pass that reads the source beside the draft, so its entire value is the comparison — and floored at `minimal` it stopped making one: a small model spent zero thinking tokens across two whole books and answered every chunk with a byte-identical empty verdict while the deterministic checks were repairing dozens of defects in the same chunks. Models that offer no `low` rung take the lowest they do offer, so the floor cannot resolve to a level the provider rejects. Gemini 2.5 editors, which were getting thinking switched off outright, now get the smallest budget instead. An explicitly chosen editor thinking level is untouched; only `auto` moves. Expect the editor to cost more per chunk: thinking tokens are now actually spent, and the automatic editor output budget doubles with the higher level.
