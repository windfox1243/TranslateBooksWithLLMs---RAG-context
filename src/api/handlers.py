...
async def perform_actual_translation(translation_id, config, state_manager, output_dir, socketio):
    ...
    _callbacks = build_job_callbacks(
        translation_id, config, state_manager, output_dir, socketio,
        checkpoint_manager,
    )
    # Unpacked into locals: the job body below already spells these names, and
    # they are read-only from here on.
    logger = _callbacks.logger
    should_interrupt_current_task = _callbacks.should_interrupt_current_task
    _log_message_callback = _callbacks.log_message_callback
    _phase_boundary_review_repair = _callbacks.phase_boundary_review_repair
    _translate_stats_callback = _callbacks.translate_stats_callback
    _refine_after_stats_callback = _callbacks.refine_after_stats_callback
    _refine_only_stats_callback = _callbacks.refine_only_stats_callback
    _finalize_stats_callback = _callbacks.finalize_stats_callback
    _openrouter_cost_callback = _callbacks.openrouter_cost_callback
