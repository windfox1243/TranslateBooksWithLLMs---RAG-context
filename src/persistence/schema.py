...
The sweep runs on every Database() construction, and must: alongside the
idempotent DDL (CREATE ... IF NOT EXISTS, PRAGMA-guarded ALTERs) it carries
data-repair passes -- the contract-v2 addressing quarantine, for one -- that
act on rows written by the current version, not only on rows inherited from
an older one. A PRAGMA user_version stamp that let a known-current file skip
the sweep would therefore skip those repairs too. Making the sweep cheap
means separating the two kinds of statement first; until then it stays whole.
"""
import hashlib
