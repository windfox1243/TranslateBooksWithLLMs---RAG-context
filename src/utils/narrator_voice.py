...
def build_narrator_voice_context(
    ...
    for item in sorted(profiles, key=lambda row: (not bool(row.get("is_locked")), row.get("narrator_key", ""))):
        ...
        lock = "locked user profile" if item.get("is_locked") else "accepted evidence profile"
        lines.append(
...
def persist_voice_observations(
    ...
    stats = {"observations": 0, "profiles": 0, "conflicts": 0}
...
async def bootstrap_narrator_voice(
    *, db: Any, translation_id: str, chunks: Sequence[Dict[str, Any]],
    ...
    """Run one bounded lazy bootstrap for a legacy job without voice state."""
    ...
    if db.get_narrator_voice_profiles(translation_id, include_inactive=True):
    ...
    indices = sorted({
        0, len(completed) - 1, len(completed) // 3,
        (2 * len(completed)) // 3,
    })[:4]
    sampled = [completed[index] for index in indices]
    sampled_indices = [int(item.get("chunk_index", 0)) for item in sampled]
    if not db.claim_narrator_bootstrap(
        translation_id, sampled_chunks=sampled_indices,
    ):
    ...
    prompt = (
        ...
        "voice_observations. Every item must include chunk_index and this contract: "
        + voice_observation_contract()
        + "\nUse exact unique source_quote and target_quote evidence. Return [] when uncertain.\n"
    ...
    try:
        ...
        db.finish_narrator_bootstrap(
            ...
            details={"accepted": len(accepted_all), "sampled": sampled_indices},
        )
        return {"status": status, "accepted": len(accepted_all)}
    except Exception as exc:
        db.finish_narrator_bootstrap(
            translation_id, "failed", details={"error": type(exc).__name__},
        )
