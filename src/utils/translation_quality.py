...
def _outside_quoted_text(text: str) -> str:
    """Return prose outside common straight and typographic quote pairs."""
    ...
    for char in str(text or ""):
        if closing is None and char in opening_to_closing:
...
def build_narrative_voice_context(
    previous_translations: Iterable[str],
    target_language: str,
) -> str:
    """Infer a high-confidence first-person narration baseline.

    Directed addressing describes dialogue pairs, not the book-wide narrator.
    This compact derived context lets the editor distinguish those two voices.
    """

    language = str(target_language or "").strip().casefold()
    if language not in {"vi", "vietnamese", "tiếng việt", "tieng viet"}:
        return ""
    prose = "\n".join(
        _outside_quoted_text(value) for value in previous_translations if value
    )
    counts = {
        pronoun: len(re.findall(
            rf"(?<!\w){re.escape(pronoun)}(?!\w)",
            prose,
            flags=re.IGNORECASE,
        ))
        for pronoun in ("tôi", "tớ", "ta")
    }
    total = sum(counts.values())
    dominant, dominant_count = max(counts.items(), key=lambda item: item[1])
    if dominant_count < 5 or total <= 0 or dominant_count / total < 0.65:
        return ""
    alternatives = ", ".join(
        f'"{key}": {value}' for key, value in counts.items() if key != dominant
    )
    return (
        "Established Vietnamese first-person narrative self-reference from "
        f'prior completed chunks: "{dominant}" ({dominant_count} '
        f"outside-dialogue occurrences; alternatives: {alternatives}). "
        f'Preserve "{dominant}" in narration and internal monologue unless '
        "the source explicitly changes viewpoint or narrative voice. "
        "Pair-specific addressing self-references apply only to dialogue or "
        "thoughts directly addressed to that pair and must not replace this "
        "narrative baseline."
    )
